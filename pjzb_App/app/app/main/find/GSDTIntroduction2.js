...
export default class GSDTIntroduction extends Component {
    constructor(props){
        super(props);
        this.state = {
            oData:[],
    ...
    _getData(flag, curPage){
        ...
        Request.post('getMediaReport.do',{curPage:curPage},(data)=>{
            if (data.error == 0) {
    ...
    onPress(title,publishTime,html,id, back) {
            ...
            <View style={styles.itemView}>
                ...
                <View style={styles.textView}>
                    <View style={styles.titleView}>
                        <Text numberOfLines={1} onPress={(html) => this.onPress(row.title,row.publishTime,c,row.id, true)} style={styles.titleText}>{row.title}</Text>
                    </View>
